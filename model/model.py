...
class Model:
    def __init__(self, num_of_states: int = 1, num_of_actions: int = 1, alfa: float = 0.1,
                 lambd: float = 0.1, actor_only: bool = False, epsilon=0.1, g_init: float = 0, n_init: float = 0):
        """
        Actor Uncertainty model
        """
        self.states = []
        for _ in range(num_of_states):
            state = State(num_of_actions=num_of_actions, alfa=alfa, lambd=lambd,
                          actor_only=actor_only, epsilon=epsilon, g_init=g_init, n_init=n_init)
